...
AlarmTime::AlarmTime(Alarm alarm, uint8_t dow)
    : alarm(alarm)
    , dow(dow)
{
}
...
{
    String s;
    s += alarm.getName() + ": " + dowName(dow) + " " + alarm.getHour() + ":" + alarm.getMinute()
        + " (" + alarm.getStream().getURL() + ")";
    return s;
}
...
MusicStream& AlarmTime::getStream() { return alarm.getStream(); }

String AlarmTime::getName() { return alarm.getName(); }

float AlarmTime::getVolume() { return alarm.getVolume(); }
...
bool AlarmTime::operator>(const AlarmTime& b) const { return this->toSec() > b.toSec(); }
